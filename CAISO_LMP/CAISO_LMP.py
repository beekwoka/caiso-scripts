...
for filename in filenames:
    ...
    with open(f"data/{filename}", 'r') as current_csv:
        print(f"Processing file {filename}...")
        #open the file as a csv
        reader = csv.DictReader(current_csv)
        # Iterate over all rows
        for row in reader:
            # Ignore all rows where LMP_Type isn't LMP
            if row['LMP_TYPE'] != "LMP":
                continue
...
with open("CAISO_LMP_output.csv", 'w') as output_csv:
    # ERIC EDIT: Remove "MW" field...
    # fieldnames = ["NODE_ID", "NODE", "MW", "AVG_LMP"]
    fieldnames = ["NODE_ID", "NODE", "AVG_LMP"]
